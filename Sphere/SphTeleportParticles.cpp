...
ParticleContext_t SphTeleportParticles::P;

SphTeleportParticles::SphTeleportParticles(cchar* directory, cchar* filename, cfloat* position)
{
...
void SphTeleportParticles::ComputeParticles(cfloat* position, cfloat* direction)
{
	P.KillOld(10.0f, false);
	    // Generate particles along a very small line in the nozzle.
	P.Source(15+Randi(10), PDCylinder(pVec(position[O_X], position[O_Y]-2.0f, position[O_Z]),pVec(position[O_X], position[O_Y]+2.0f, position[O_Z]),2.0f+Randf(),1.0f));
...
void SphTeleportParticles::Render()
{
    ...
    int num_ret = P.GetParticles(0, cnt, ppos);
        ...
        glPushMatrix();
		glEnable(GL_BLEND);
		glAlphaFunc ( GL_GREATER,(GLclampf) 0.0000001 ) ;
